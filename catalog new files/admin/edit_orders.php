...
*/
  ...
  if (! defined('ORDER_EDITOR_USE_AJAX')) { // if configs not installed
    ...
    $sql_data_array = array(
          'configuration_group_title' => 'Order Editor',
          'configuration_group_description' => 'Configuration options for Order Editor.',
    );
  ...
  $settings = array(
    'ORDER_EDITOR_PAYMENT_DROPDOWN' => array(
                   'title' => 'Display the Payment Method dropdown?',
                   'desc' => 'Based on this selection Order Editor will display the payment method as a dropdown menu (true) or as an input field (false).',
                   'value' => 'true',
                   'sort' => 1,
                   'set_func' => 'tep_cfg_select_option(array(\'true\', \'false\'),'), 
    'ORDER_EDITOR_USE_SPPC' => array(
                   'title' => 'Use prices from Separate Pricing Per Customer?',
                   'desc' => 'Leave this set to false unless SPPC is installed.',
                   'value' => 'false',
                   'sort' => 10,
                   'set_func' => 'tep_cfg_select_option(array(\'true\', \'false\'),'), 
    'ORDER_EDITOR_USE_QTPRO' => array(
                   'title' => 'Use QTPro contribution?',
                   'desc' => 'Leave this set to false unless you have QTPro Installed.',
                   'value' => 'false',
                   'sort' => 20,
                   'set_func' => 'tep_cfg_select_option(array(\'true\', \'false\'),'), 
    'ORDER_EDITOR_USE_AJAX' => array(
                   'title' => 'Allow the use of AJAX to update order information?',
                   'desc' => 'This must be set to false if using a browser on which JavaScript is disabled or not available.',
                   'value' => 'true',
                   'sort' => 30,
                   'set_func' => 'tep_cfg_select_option(array(\'true\', \'false\'),'), 
    'ORDER_EDITOR_CREDIT_CARD' => array(
                   'title' => 'Select your credit card payment method',
                   'desc' => 'Order Editor will display the credit card fields when this payment method is selected.',
                   'value' => 'Credit Card',
                   'sort' => 40,
                   'set_func' => 'tep_cfg_pull_down_payment_methods('), 
    'ORDER_EDITOR_ADD_PDF_INVOICE_EMAIL' => array(
                   'title' => 'Attach PDF Invoice to New Order Email',
                   'desc' => 'When you send a new Order Email a PDF Invoice kan be attach to your email. This function only works if the contribution PDF Invoice is installed.',
                   'value' => 'false',
                   'sort' => 50,
                   'set_func' => 'tep_cfg_select_option(array(\'true\', \'false\'),'), 
    'ORDER_EDITOR_LIST_DIRECT_TO_EDITOR' => array(
                   'title' => 'Change order list Edit button',
                   'desc' => 'When clicking on Edit button in order list info box, go straight to Order Editor?',
                   'value' => 'true',
                   'sort' => 60,
                   'set_func' => 'tep_cfg_select_option(array(\'true\', \'false\'),'), 
    ...
    foreach($settings as $key => $data) {
      if (! defined($key)) {
            ...
            if (isset($data['sort'])) {
              $sql_data_array['sort_order'] = $data['sort'];
            }
  ...
  if ($new_settings) {
    $messageStack->add_session(SUCCESS_CONFIGS_INSTALLED,'success');
    tep_redirect('configuration.php?gID=' . $group_id);
  }
...
//					   EMAIL_TEXT_DATE_ORDERED . ' ' . tep_date_long($check_status['date_purchased']) . "\n\n" . sprintf(EMAIL_TEXT_STATUS_UPDATE, $orders_status_array[$status]) . $notify_comments . sprintf(EMAIL_TEXT_STATUS_UPDATE2);
	     ...
	     if (FILENAME_EMAIL_STATUS !== 'FILENAME_EMAIL_STATUS'     ) {
		   ...
		   if (EMAIL_USE_HTML == 'true') {	
		     ...
		     //Prepare HTML email
			 require('includes/modules/email/html_orders.php');
...
// QT Pro Addon BOF	
					...
					$products_stock_attributes = implode($products_stock_attributes, ',');
					 // update the stock
					 tep_db_query("update products_stock set products_stock_quantity = products_stock_quantity - ".$quantity_difference . " where products_id= '" . $order_products['products_id'] . "' and products_stock_attributes='".$products_stock_attributes."'");				 
					}
// QT Pro Addon EOF
				 } else {
					tep_db_query ("UPDATE products SET
...
// QT Pro Addon EOF
              ...
              // Update Any Attributes
				...
				if(isset($products_details['attributes'])) { 
				  foreach($products_details['attributes'] as $orders_products_attributes_id => $attributes_details) {
					$Query = "UPDATE orders_products_attributes set
      ...
      if (isset($_POST['update_downloads']) && is_array($_POST['update_downloads'])) {
		      ...
		      if (is_array($_POST['update_comments'])) {
	              foreach($_POST['update_comments'] as $orders_status_history_id => $comments_details) {
	                  ...
	                  if (isset($comments_details['delete'])){
			             ...
			             $Query = "DELETE FROM orders_status_history 
			                              WHERE orders_id = '" . (int)$oID . "' 
		                 ...
		                 $Query = "UPDATE orders_status_history SET
      ...
      if (isset($_POST['update_totals']) && is_array($_POST['update_totals'])) {
...
// eof order editor 5_0_8
  ...
  $currency_query = tep_db_query("select distinct title, code from currencies order by code ASC");  
 ...
 ?>
      ...
      <div id="header">
        ...
        document.write("<li><span class=\"tdbLink\"><a id=\"tdb0\" href=\"javascript:newOrderEmail()\" class=\"ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary ui-priority-secondary\" role=\"button\" aria-disabled=\"false\"><span class=\"ui-button-icon-primary ui-icon ui-icon-mail-closed\"></span><span class=\"ui-button-text\"><?php echo IMAGE_NEW_ORDER_EMAIL ;?></span></a></span></li>");
			...
			<li><?php echo tep_draw_button('Details', 'document', tep_href_link('orders.php', tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $_GET['oID'] . '&action=edit')); ?></li>
...
<!- eof 5.0.8 -->							
			...
			<li><?php echo tep_draw_button(IMAGE_BACK, 'arrow-1-w', tep_href_link('orders.php', tep_get_all_get_params(array('action'))), null)?></li>
		  </ul>
        ...
        <table border="0" cellspacing="0" cellpadding="2">
          <tr>
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="2" style="border: 1px solid #C9C9C9;">
              ...
              <tr class="dataTableRow"> 
                <td class="dataTableContent" valign="middle" align="right"><?php echo (defined('ENTRY_TELEPHONE_NUMBER') ? ENTRY_TELEPHONE_NUMBER : ENTRY_TELEPHONE); ?></td>
