...
*/
  ...
  class manualOrder {
    ...
    function query($order_id) {
      ...
      $totals_query = tep_db_query("select * from orders_total where orders_id = '" . (int)$order_id . "' order by sort_order");
      ...
      $index = 0;
      $orders_products_query = tep_db_query("select orders_products_id, products_id, products_name, products_model, products_price, products_tax, products_quantity, final_price from orders_products where orders_id = '" . (int)$order_id . "' order by orders_products_id");
	
      while ($orders_products = tep_db_fetch_array($orders_products_query)) {
        
        $orders_products_tax_query = tep_db_query("select products_tax_class_id from products where products_id = " . $orders_products['products_id'] . "");
        ...
        $subindex = 0;
        $att_array = [];
        $attributes_query = tep_db_query("select products_options, products_options_values, options_values_price, price_prefix, orders_products_attributes_id from orders_products_attributes where orders_id = '" . (int)$order_id . "' and orders_products_id = '" . (int)$orders_products['orders_products_id'] . "'");
        ...
        $this->products[$index]['id'] = tep_get_uprid($orders_products['products_id'], $att_array);
    ...
    function adjust_zones() {
      ...
      $totals_query = tep_db_query("select orders_total_id, title, text, class, value from orders_total where orders_id = '" . (int)$order_id . "' order by sort_order");
      $this->totals = array();
      $i=0;
      while ($totals = tep_db_fetch_array($totals_query)) {
